...
- Eqs 2.7, 2.8 and 4.5 are not approximations but exact. Make appropriate changes in text and equations. For example, change the first sentence of 2.3 from "Even though in the case of the bivariate Gaussian no closed-form solution exists for the distribution of z given that x is below or above a threshold, there is an approximation formula for ..." to "If $p[x,z]$ is a bivariate Gaussian, then $p[z|x]$ is a Gaussian too, but the two distributions that we need for PRA, i.e. $p[z|x<thr]$ and $p[z| \ge thr]$, are not Gaussian. However there are closed-form solutions for the means and variances of these two distributions (e.g. @Mee_1983_Simple), which we show here in their most general form."
...
```{r}
  ...
  curve( 1-exp(-x), xlim=c(0,3), ylab="", main="E[z|x]",
         cex.main=2, cex.axis=2, cex.lab=2, lwd=2 )
```
...
```{r, echo=F}
  ...
  for(i in 1:np) {
    u.tbl1 <- rbind( u.tbl1,
      u( a.seq, x.smp1[i], t.smp1[i], e.smp1[i], ka.smp1[i], ky.smp1[i] ) )
    u.tbl2 <- rbind( u.tbl2,
      u( a.seq, x.smp2[i], t.smp2[i], e.smp2[i], ka.smp2[i], ky.smp2[i] ) )
    u.tbl3 <- rbind( u.tbl3,
      u( a.seq, x.smp3[i], t.smp3[i], e.smp3[i], ka.smp3[i], ky.smp3[i] ) )
  }
