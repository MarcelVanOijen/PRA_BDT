...
- GitHub: **https://github.com/MarcelVanOijen/PRA_BDT**

## References

- Main references: **@VanOijen_2022_Probabilistic** ('PRA-BDT') and @VanOijen_2024_Bayesian chapters 18 and 19 ('BC2').
- Other previous publications: @VanOijen_2019_Probabilistic, @VanOijen_2019_Tools, @VanOijen_2014_Impact, @VanOijen_2013_novel.
- Some publications that use (parts of) our PRA approach explicitly: @Ren_2023_Risk, @Nandintsetseg_2021_Risk, @He_2021_Drought, @Zhou_2018_Drought, @Kuhnert_2017_Impact, @Liu_2020_new, @Nandintsetseg_2024_Future.

### @VanOijen_2022_Probabilistic: Errata

- p.14: the text refers to 'Monte Carlo' (MC) four times but the approach is based on numerical solutions to the conditional expectation values without any MC sampling.
- p.21, 4.3: "any deviation ... $E[z \geq thr]$" should be ... $E[z|x \geq thr]$. 
- pp.13-14,27, Eqs 2.7, 2.8 and 4.5: these are not approximations but exact, so appropriate changes in equations and text must be made.
- p.27: "$Var[z|x < thr] = 1 + \rho \, thr \, E[z] - E[z]^2$" should have conditional expectations on the right: "$... = 1 + \rho \, thr \, E[z|x < thr] - E[z|x<thr]^2$".
- p.27,Eq.4.5: The equations as written give a value equal to 1 for the conditional variances if $\rho = 0$ and $thr \to \infty$ resp. $thr \to -\infty$. The proper limits are $\sigma^2_z$ so that is what both appearances of "$1$" should be replaced by.
- p.59, Eq.11.1: the four $R_e$ should each be $Q$.

## Agenda
We cover the following six topics:
...
With four sessions of 1.5 hours each, a possible schedule could be:
...
We include exercises in each session to have some (active) breaks and to initiate discussion.
...
We use several virtual datasets and one real one in this notebook.
...
```{r, eval=F}
  ...
  # Annual mortality and summer rain of last three years
  ...
  save( x_r3, z_Fs, z_Q, z_Pa, z_Ps, file=paste0(d.data,"xz_Germany.RData" ) )
```
...
```{r, fig.height=4}
  ...
  pairs(xz_G3, diag.panel=panel.hist, label="" ) ; round( cor(xz_G3), 3 )
```
...
Here is a plotting function for those cases where we have, in one vector, all six values of a standard sampling-based single-threshold PRA: $pH,V,R,s_{pH},s_V,s_R$.
...
We use the first nonlinear dataset to see an example of multi-threshold PRA in action.
...
```{r PRAmNL, echo=F, out.width="80%", fig.height=3.5, fig.align="center", fig.cap="Multi-threshold PRA on the first nonlinear dataset."}
...
Here are quick checks for the sparse dataset, the first linear one, and the first nonlinear one: 
...
This section deals only with the bivariate Gaussian p[x,z] for which formulas for the conditional means and variances were given by @VanOijen_2022_Probabilistic Eqs 2.7 and 4.5. Here is a new implementation of those formulas (including an error correction for the conditional variance: "1 + .." becoming "Vz + .."):
...
- Eq. 2.7 =>  E[z|z<thr]   = mz - Vz * pthr / Fthr
- Eq. 4.5 =>  Var[z|z<thr] = Vz - (thr-mz) * Vz * pthr/Fthr - Vz^2 * (pthr/Fthr)^2
...
## PRA
The above formulas for conditional means and variances of the bivariate Gaussian lead to very simple equations for PRA (@VanOijen_2022_Probabilistic p.14, Eqs 2.8), which we implement in the following R-function: 
...
So whenever we are certain that the joint distribution $p[x,z]$ is well represented by $N[\mu,\Sigma]$, the function call $\texttt{PRA0\_Gauss(mu,Sigma,thr)}$ would give us our PRA. But generally we only have a sample from $\{x,z\}$ and even if we are convinced that a bivariate Gaussian is suitable, we then still have uncertainty about the hyperparameters of the distribution. So we need a probability distribution for the hyperparameters. For this, we use a conjugate Bayesian approach where the posterior distribution can be calculated analytically from the sample mean vector $m$ and covariance matrix $S$.

Say that our data are $y=\{x_i,z_i\}, i=1..n$ with $m = (\overline{x},\overline{z})$ and $S = cov(x,z)$. If we assign an uninformative Jeffreys prior $p[\mu,\Sigma] \propto |\Sigma|^{-3/2}$, then the posterior is an inverse Wishart distribution for $\Sigma$ times a conditionally normal distribution for $\mu$ (e.g. @Gelman_2013_Bayesian). Concretely, $p[\Sigma|y] = IWi_{n-1}(S'^{-1})$ and $p[\mu|\Sigma,y] = N[m,\Sigma/n]$, where $S'=(n-1)S$ is the sample sum of squares matrix.

(The bivariate Gaussian has dimension $d=2$. What would happen if we had higher-dimensional data? Well, conjugacy would still apply if the Jeffreys prior for the hyperparameters would then be $p[\mu,\Sigma] \propto |\Sigma|^{-(d+1)/2}$. An example would be a trivariate Gaussian $y = \{x_1,x_2,z\}$ with as prior $p[\mu,\Sigma] \propto |\Sigma|^{-2}$.)
...
$`r m2l( round( riwish(n-1,S*(n-1)), 2 ) )`$
$`r m2l( round( riwish(n-1,S*(n-1)), 2 ) )`$
$`r m2l( round( riwish(n-1,S*(n-1)), 2 ) )`$
...
In that code we wrote the conditional variances as functions of $R$ and $V$ which draws upon the following equivalences for bivariate Gaussian conditional expectations and variances:

\begin{equation}
\begin{aligned}
  E[z|x < thr]      &= E[z] - \, \rho \, \sigma_x \, \sigma_z \,
                         \frac{p_x[thr]}{F_x[thr]} \\
                    &= E[z] + R - V. \\
  E[z|x \geq thr]   &= E[z] + \, \rho \, \sigma_x \, \sigma_z \,
                         \frac{p_x[thr]}{1-F_x[thr]} \\
                    &= E[z] + R. \\
  Var[z|x < thr]    &= \sigma_z^2 + \rho \, (thr-E[x]) \, (E[z|x < thr] - E[z]) -
                         (E[z|x < thr] - E[z])^2 \\
                    &= \sigma_z^2 + \rho \, (thr-E[x]) \, (R-V) - (R-V)^2. \\
  Var[z|x \geq thr] &= \sigma_z^2 + \rho \, (thr-E[x]) \, (E[z|x \geq thr] - E[z]) -
                         (E[z|x \geq thr] - E[z])^2 \\
                    &= \sigma_z^2 + \rho \, (thr-E[x]) \, R - R^2.
\end{aligned}
\end{equation}
...
Note: even in the case of the trivariate Gaussian (and zero-thresholds), p[ x2 | x1<0 ] is not a Gaussian. To understand the results of the PRAi on the trivariate Gaussian dataset, it is still useful to derive a formula for the probability of joint hazardousness p[x2<0|x1<0], i.e. the joint cumulative distribution function. Check @Mee_1983_Simple for an estimate of this. Continue that line of thinking to develop distribution-based multi-hazard PRA.
...
@VanOijen_2022_Probabilistic gave an example of single-threshold, distribution-based continuous PRA in Chapter 9.

## Zero-threshold continuous PRA
@VanOijen_2022_Probabilistic further suggested (on p.7) that zero-threshold continuous PRA is feasible if we assume that the function $z(x)$ has a known maximum. We show an example of that approach now, for model-based PRA.

Our example will be similar to the second of the two examples shown in the chapter on model-based PRA of @VanOijen_2022_Probabilistic (p.42-43). Here are the equations that capture our knowledge about $\{x,z\}$. (We ignore UQ in this example.)
...
The model-based continuous zero-threshold PRA-equations then become:
...
## Loss distribution
We now show that continuous-PRA allows deriving a continuous probability distribution for expected loss, and for that we give a distribution-based PRA example.
...
```{r}
           PRA(  x_r3, 100-z_Pa,  thr )   # Standard approach here
           PRA(  x_r3,    -z_Pa,  thr )
           PRA(  x_r3,     z_Pa,  thr )
           PRA( -x_r3,     z_Pa, -thr )
  
  plotPRA( PRA(  x_r3, 100-z_Pa,  thr ) ) # Standard approach here
  plotPRA( PRA(  x_r3,    -z_Pa,  thr ) )
  plotPRA( PRA(  x_r3,     z_Pa,  thr ) )
  plotPRA( PRA( -x_r3,     z_Pa, -thr ) )
```
...
```{r, echo=F, eval=F}
  ...
  names(s_prec.mon_DEU) <- stringr::str_sub( names(s_prec.mon_DEU), -7, -1 )
...
```{r, echo=T}
  kz <- 30 ; r_U_DEU <- r_YC_DEU * kz ; r_U_em_DEU <- r_YC_em_DEU * kz
```
...
```{r, echo=T}
  ...
  r_U_em.IRRIG_DEU  <- r_YC_em.IRRIG_DEU * kz - IRRIG * ka
```
...
## A simple continuous example of BDT in action

The following R-code chunks are minor modifications of code for Chapter 12.3 of @VanOijen_2022_Probabilistic, for example writing $z$ instead of $y$.
...
Acquiring weather data remains problematic. The following methods provide some access but with various limitations.
