...
## Formulas for the conditional bivariate Gaussian expectations

\begin{equation}
\begin{aligned}
  &\underline{ \textrm{EXPECTATIONS FOR BIVARIATE GAUSSIAN p[x,z]:} } \\
...
```{r EzcondGaussian, echo=F, out.width="50%", fig.height=6, fig.align="center", fig.cap="Conditional expectations for $z$ as a function of threshold-value in the case of bivariate Gaussian $p[x,z]$. Solid lines: Numerical integration. Dashed lines: Formulas."}
  thr.seq       <- seq( mx-2, mx+2, length.out=101 )
  ...
  legend( "bottomright",
          legend=c( "E[z|x>=thr] (num. int.)", "E[z|x>=thr] (formula)",
                    "E[z|x<thr] (num. int.)" , "E[z|x<thr] (formula)" ),
...
\begin{equation}
\begin{aligned}
  &\underline{ \textrm{PRA FOR BIVARIATE GAUSSIAN p[x,z]:} } \\
  &p[H] = F_x[thr], \\
  &V    = \frac{\rho \, \sigma_x \, \sigma_z \, p_x[thr]}
                     {F_x[thr] \, (1-F_x[thr])}, \\
  &R    = \frac{\rho \, \sigma_x \, \sigma_z \, p_x[thr]}
                     {1-F_x[thr]}.
  (\#eq:PRAGaussian)
\end{aligned}
\end{equation}
...
## Formulas for the conditional bivariate Gaussian variances 

\begin{equation}
\begin{aligned}
  &\underline{ \textrm{VARIANCES OF BIVARIATE GAUSSIAN p[x,z]:} } \\
  &Var[z|x < thr]    = \sigma_z^2 + \rho \, (thr-E[x]) \, (\mu_1 - E[z]) - (\mu_1 - E[z])^2, \\
  &Var[z|x \geq thr] = \sigma_z^2 + \rho \, (thr-E[x]) \, (\mu_2 - E[z]) - (\mu_2 - E[z])^2,
  (\#eq:VarzcondGaussian)
\end{aligned}
\end{equation}

where $\mu_1 = E[z|x < thr]$ and $\mu_2 = E[z|x \geq thr]$ for which the formulas were given in Eq. \@ref(eq:EzcondGaussian).
